...
public class Main {
    ...
    public static ArrayList<String> duplicateUpperEnd(ArrayList<String> wordList) {
        int constantSize = wordList.size();
    ...
    public static ArrayList<String> moveBWords(ArrayList<String> wordList) {
        // Ensure that the words with b are in order by adding it first at zero (which pushes the other items to the right) and then increasing the c value
        int c = 0;
    ...
    public static ArrayList<Integer> removeDuplicates(ArrayList<Integer> intList) {
        ...
        for (int i = 0; i < intList.size(); i++) {
            ...
            if (map.get(intList.get(i)) == null) {
                map.put(intList.get(i), true);
            } else {
                intList.remove(i);
                i--;
            }
    ...
    public static ArrayList<Integer> notAlone(ArrayList<Integer> list, int val) {
        ...
        for (int i = 0; i < list.size(); i++) {
            int curr = list.get(i);
            if (curr == val) {
                // We know that list.get(1) will exist because of the above if statemen so dont
                // check anything for that except i== 0
                if (i == 0) {
                    ...
                    if (list.get(i - 1) > list.get(i + 1)) {
                        ...
                        // Just set any if they are same value
                        list.set(i, list.get(i - 1));
                    }
                }
            }
    ...
    public static boolean addNext(ArrayList<Integer> list, int startIndex, int val, int addIndex) {
        for (int i = startIndex + 1; i < list.size(); i++) {
            if (list.get(i) == val) {
                list.remove(i);
                list.add(addIndex, val);
                // Return to save performance (task ended)
                return true;
            }
        }
        return false;
    ...
    public static ArrayList<Integer> fix34(ArrayList<Integer> list) {
        for (int i = 0; i < list.size(); i++) {
            // Even though this is bad for performance, it makes sense logically :)
            if (list.get(i) == 4) {
                // System.out.println("found4");
                // Move to the end so the addNext method will work correctly
                list.remove(i);
                list.add(4);
                addNext(list, i - 1, 3, i + 1);
            } else if (list.get(i) == 3) {
                // System.out.println("addNext");
                // Look for a 4 ahead of current index
                if (!addNext(list, i, 4, i + 1)) {
                    // If it failed, then just return since you can't do arrange more
                    return list;
                } else {
                    i++;
                }
    ...
    public static ArrayList<Integer> modes(int[] numList) {
        ...
        for (int i = 0; i < numList.length; i++) {
            if (map.get(numList[i]) == null) {
                map.put(numList[i], 1);
            } else {
                map.put(numList[i], map.get(numList[i]) + 1);
            }
        ...
        if (!overrideSame && same == map.size() - 1) {
